...
function formatTime(timestamp: string): string {
  ...
  const [messages, setMessages] = useState<{ id: string; senderId: string; text: string; createdAt: string; isRead: boolean }[]>([]);
      ...
      <div className="max-h-[80vh]   flex flex-col justify-center items-center">
        <div className="bg-white py-8 px-4 shadow-2xl sm:rounded-lg sm:px-10 w-[80vw] border flex flex-col">
          ...
          <div className="mt-8 flex">
            ...
            <button
              type="submit"
              className="bg-[#1E3A45] text-white rounded-full px-4 py-2"
              onClick={sendMessage}
            >
              <FaRegPaperPlane />
            </button>
